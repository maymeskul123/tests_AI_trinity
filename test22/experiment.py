...
# ============================================================
# TEST 22 — CONFLICT КАК ТРИГГЕР ДЛЯ ОГРАНИЧЕННОГО РЕСУРСА
#
# Источник A: conflict_rate = 0.4, неограничен
# Источник B: conflict_rate = 0.0, но только 2 запроса
# Quaternary использует CONFLICT для переключения на Б.
# ============================================================
...
MAX_B_USAGE = 2   # ограничение на число запросов к источнику Б
...
class AgentBase:
    ...
    def choose_question_and_source(self, sources, b_limit):
        raise NotImplementedError
...
class BinaryAgent(AgentBase):
    ...
    def choose_question_and_source(self, sources, b_limit):
        ...
        for idx, s in enumerate(sources):
            # Если это источник Б и лимит исчерпан — пропускаем
            if idx == 1 and self.b_used >= b_limit:
                continue
            for q in range(N_Q):
...
# ------------------------------------------------------------
# QUATERNARY (с переключением по CONFLICT)
# ------------------------------------------------------------
class QuaternaryAgent(AgentBase):
    ...
    def choose_question_and_source(self, sources, b_limit):
        ...
        if conflict_candidates:
            # Если есть конфликт, пытаемся использовать Б (если лимит не исчерпан)
            if self.b_used < b_limit:
                ...
                if best_q is not None:
                    return 1, best_q  # источник Б
            # Если лимит исчерпан, остаёмся на А (или воздерживаемся)
            # Выбираем лучший вопрос на А
            prior = [1.0/len(active) if h in active else 0.0 for h in HYPOTHESES]
            best_gain = -1
            best_q = None
            for q in range(N_Q):
                gain = expected_information_gain(prior, q, sources[0].conflict_rate)
                if gain > best_gain:
                    best_gain = gain
                    best_q = q
            if best_q is not None:
        ...
        # Нет конфликтов — стандартный выбор (предпочитаем А, т.к. Б ограничен)
        prior = [1.0/len(active) if h in active else 0.0 for h in HYPOTHESES]
        best_gain = -1
        best_pair = (None, None)
        for idx, s in enumerate(sources):
...
def run_single_episode(AgentClass, seed, budget, conflict_A):
    ...
    for step in range(budget):
        ...
        src_idx, q = agent.choose_question_and_source(sources, MAX_B_USAGE)
...
def main():
    ...
    for conflict_A in CONFLICT_RATES_A:
        print(f"\n=== conflict_A = {conflict_A:.1f} (лимит Б = {MAX_B_USAGE}) ===")
    ...
    path = "results/test22/results.csv"
