...
#
# Гипотеза: добавление AMBIGUOUS снижает воздержание и повышает точность
# при сохранении нулевой ложной уверенности.
# ============================================================
...
# ============================================================
# QUINTARY AGENT (ДОБАВЛЕНО AMBIGUOUS)
# ============================================================
class QuintaryAgent(AgentBase):
    def reset(self):
        self.states = [UNKNOWN] * N_H
    ...
    def choose_question(self, conflict_rate):
        ...
        # Приоритет для AMBIGUOUS и CONFLICT
        priority = [h for h in active if self.states[h] in (CONFLICT, AMBIGUOUS)]
        if priority:
            best_score = -1
            best_q = None
            for q in range(N_Q):
                split = sum(1 for h in priority if QUESTIONS[q][h] == 1)
                score = split / len(priority)
                if score > best_score:
                    best_score = score
                    best_q = q
            if best_q is not None:
    ...
    def observe(self, q, value):
        for h in HYPOTHESES:
            ...
            expected = QUESTIONS[q][h]
            if expected == value:
                # Совпало: CONFLICT или AMBIGUOUS -> RIGHT, иначе RIGHT
                self.states[h] = RIGHT
            else:
                # Не совпало
                if state == RIGHT:
...
def main():
    ...
    path = "results/test24/results.csv"
