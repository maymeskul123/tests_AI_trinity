...
# ============================================================
# TEST26 — ГИБРИДНЫЙ АГЕНТ (TERNARY + QUATERNARY_THRESHOLD)
#
# Переключается между Ternary и Threshold-логикой
# в зависимости от числа гипотез в CONFLICT.
# ============================================================
...
# Порог для переключения: если > 2 гипотез в CONFLICT — используем Threshold
SWITCH_THRESHOLD = 2
CONFLICT_THRESHOLD = 3   # для Threshold-режима (количество несовпадений до WRONG)
...
class HybridAgent(AgentBase):
    def reset(self):
        self.states = [UNKNOWN] * N_H
        self.counter = [0] * N_H   # для threshold-режима

    def choose_question(self, conflict_rate):
        # Используем логику Ternary, но с приоритетом для CONFLICT, если их много
        active = [h for h in HYPOTHESES if self.states[h] != WRONG]
        if len(active) <= 1:
            return None
        confirmed = [h for h in active if self.states[h] == RIGHT]
        ...
        # Проверяем, нужно ли переключиться в Threshold-режим
        conflict_count = sum(1 for h in active if self.states[h] == CONFLICT)
        self.use_threshold = conflict_count > SWITCH_THRESHOLD

        # Если много конфликтов — приоритет вопросам, разделяющим их
    ...
    def observe(self, q, value):
        ...
        active = [h for h in HYPOTHESES if self.states[h] != WRONG]
        conflict_count = sum(1 for h in active if self.states[h] == CONFLICT)
        use_threshold = conflict_count > SWITCH_THRESHOLD
...
def main():
    ...
    path = "results/test26/results.csv"
