...
# ============================================================
# TEST27 — АДАПТИВНЫЙ ПОРОГ КОНФЛИКТА
#
# Порог перехода CONFLICT -> WRONG вычисляется динамически
# на основе частоты несовпадений в последних наблюдениях.
# ============================================================
...
# ------------------------------------------------------------
# Базовые агенты (Binary, Ternary, Quaternary, Threshold)
# ------------------------------------------------------------
...
# Адаптивный порог
# ------------------------------------------------------------
class AdaptiveThresholdAgent(AgentBase):
    def reset(self):
        self.states = [UNKNOWN] * N_H
        self.counter = [0] * N_H
        self.history = deque(maxlen=WINDOW_SIZE)   # храним 1 если несовпадение, 0 если совпадение

    def compute_threshold(self):
        if len(self.history) == 0:
            return 2   # начальный порог
        conflict_rate_est = sum(self.history) / len(self.history)
        # Порог = max(1, round(conflict_rate_est * 5 + 1))
        threshold = max(1, round(conflict_rate_est * 5 + 1))
        return threshold
    ...
    def observe(self, q, value):
        # Получаем ожидаемые ответы для всех гипотез (не для конкретной цели)
        # Для оценки шума мы смотрим, совпало ли наблюдение с ответом правильной гипотезы.
        # Но у нас нет правильной гипотезы, поэтому будем использовать информацию о том,
        # было ли несовпадение для какой-либо гипотезы в состоянии RIGHT.
        # Упрощённо: считаем, что если для всех гипотез RIGHT наблюдение не совпало,
        # то это конфликт. Но это не точно. Лучше считать конфликты на уровне гипотез.
        # Мы просто будем запоминать, было ли несовпадение для текущего вопроса
        # относительно ожидаемого ответа для гипотезы, которая сейчас в RIGHT (если такая есть).
        # Если нет RIGHT гипотез, то считаем, что конфликта не было.
        conflict_occurred = 0
        right_hypotheses = [h for h in HYPOTHESES if self.states[h] == RIGHT]
        if right_hypotheses:
            # Проверяем, совпало ли наблюдение с ответом хотя бы для одной RIGHT гипотезы
            match_found = any(QUESTIONS[q][h] == value for h in right_hypotheses)
            if not match_found:
                conflict_occurred = 1
        # Если нет RIGHT гипотез, считаем, что конфликта не было (мы в начальной фазе)
        self.history.append(conflict_occurred)
        ...
        for h in HYPOTHESES:
            ...
            if expected == value:
                ...
                if state == RIGHT:
                    ...
                    self.counter[h] += 1
                    if self.counter[h] >= current_threshold:
                        self.states[h] = WRONG
                else:
                    self.states[h] = WRONG
...
def main():
    ...
    for conflict_rate in CONFLICT_RATES:
        ...
        for budget in BUDGETS:
            for name, AgentClass in [("BINARY", BinaryAgent),
                                     ...
                                     ("ADAPTIVE_THRESHOLD", AdaptiveThresholdAgent)]:
    ...
    path = "results/test27/results.csv"
