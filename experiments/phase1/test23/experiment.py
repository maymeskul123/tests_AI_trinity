...
# ============================================================
# TEST23 — ВЫСОКИЙ ШТРАФ ЗА ЛОЖНУЮ УВЕРЕННОСТЬ
#
# Проверяем, что Quaternary даёт преимущество в задачах,
...
# Награда: высокий штраф за ложную уверенность
REWARD_CORRECT = 1.0
REWARD_FALSE = -100.0
REWARD_ABSTAIN = -0.2
QUERY_COST = 0.05
...
class QuaternaryAgent(AgentBase):
    ...
    def choose_question(self, conflict_rate):
        ...
        if conflict_candidates:
            # Приоритет разрешению конфликтов
            best_score = -1
            best_q = None
            for q in range(N_Q):
...
def main():
    ...
    path = "results/test23/results.csv"
