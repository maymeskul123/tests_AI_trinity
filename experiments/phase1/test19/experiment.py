...
# ============================================================
# TEST 19 — ЧИСТЫЙ ПРИНЦИП (БЕЗ ЭКОНОМИКИ)
#
# Источник A: conflict_rate = 0.4
# Источник B: conflict_rate = 0.0 (идеальный)
# Стоимость отсутствует.
# Агенты могут свободно выбирать источник.
# ============================================================
...
class QuaternaryAgent(AgentBase):
    ...
    def choose_question_and_source(self, sources):
        ...
        if conflict_candidates:
            # Переключаемся на источник с наименьшим conflict_rate (идеальный)
        ...
        # Нет конфликтов — стандартный выбор по информативности
        prior = [1.0/len(active) if h in active else 0.0 for h in HYPOTHESES]
        best_gain = -1
        best_pair = (None, None)
        for idx, s in enumerate(sources):
        ...
        for h in HYPOTHESES:
            ...
            if expected == value:
                if state == CONFLICT:
                    ...
                    # WRONG не может стать RIGHT — оставляем WRONG
                    pass
                else:  # UNKNOWN или RIGHT
                    self.states[h] = RIGHT
            else:
                if state == RIGHT:
...
# Experiment
# ------------------------------------------------------------
def run_episode(AgentClass, seed, budget, conflict_A):
...
def main():
        ...
        for budget in BUDGETS:
            for name, AgentClass in [("BINARY", BinaryAgent),
                ...
                for ep in range(EPISODES):
                    ...
                    rows.append(run_episode(AgentClass, seed, budget, conflict_A))
                res = aggregate(rows)
    ...
    path = "results/test19/results.csv"
