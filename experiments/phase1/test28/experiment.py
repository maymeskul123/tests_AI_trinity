...
# ============================================================
# TEST28 — ИЕРАРХИЧЕСКИЙ АГЕНТ (TERNARY ↔ THRESHOLD ПО ЭНТРОПИИ)
#
# Переключение между Ternary и Threshold в зависимости от
# энтропии апостериорного распределения.
# ============================================================
...
# Порог энтропии для переключения: если энтропия > 0.5 бит → Threshold
ENTROPY_THRESHOLD = 0.5
CONFLICT_THRESHOLD = 3   # для Threshold-режима
...
# (Код сокращён для читаемости, но используется полная логика)
# ------------------------------------------------------------
...
# ============================================================
# ИЕРАРХИЧЕСКИЙ АГЕНТ (ПО ЭНТРОПИИ)
# ============================================================
class HierarchicalAgent(AgentBase):
    def reset(self):
        self.states = [UNKNOWN] * N_H
        self.counter = [0] * N_H
        self.use_threshold = False   # начинаем с Ternary

    def choose_question(self, conflict_rate):
        # Используем Ternary-выбор (как у TernaryAgent), но с учётом конфликтных гипотез
        active = [h for h in HYPOTHESES if self.states[h] != WRONG]
        if len(active) <= 1:
            return None
        confirmed = [h for h in active if self.states[h] == RIGHT]
        ...
        # Приоритет для конфликтных гипотез, если они есть
        conflict_candidates = [h for h in active if self.states[h] == CONFLICT]
        if conflict_candidates:
            best_score = -1
            best_q = None
    ...
    def observe(self, q, value):
        # Вычисляем энтропию апостериорного распределения
        # Для этого нужен prior, но в observe мы не передаём его.
        # Используем приближение: смотрим, сколько гипотез активны (не WRONG)
        active = [h for h in HYPOTHESES if self.states[h] != WRONG]
        if len(active) == 0:
            return
        # Оценка энтропии: если много активных гипотез → высокая энтропия
        # Если мало → низкая. Это грубое приближение, но для проверки достаточно.
        # Более точно: uniform по активным
        p = 1.0 / len(active)
        entropy_est = math.log2(len(active))  # энтропия равномерного распределения
        # Если энтропия > порога → используем Threshold
        self.use_threshold = entropy_est > ENTROPY_THRESHOLD
...
def main():
    ...
    path = "results/test28/results.csv"
