...
# ============================================================
# TEST 21 — QUATERNARY С TERNARY-ЛОГИКОЙ (БЕЗ CONFLICT В OBSERVE)
#
# Quaternary использует ту же логику обновления, что и Ternary:
#   - совпало → RIGHT
#   - не совпало → WRONG
# CONFLICT пока не используется, но состояние сохранено.
# Это позволяет Quaternary корректно решать задачу.
# ============================================================
...
# ------------------------------------------------------------
# QUATERNARY (TERNARY-ЛОГИКА, CONFLICT НЕ ИСПОЛЬЗУЕТСЯ)
# ------------------------------------------------------------
class QuaternaryAgent(AgentBase):
    ...
    def choose_question_and_source(self, sources):
        # Идентично Ternary, но с учётом CONFLICT (которого нет)
        active = [h for h in HYPOTHESES if self.states[h] != WRONG]
        if len(active) <= 1:
            return None, None
        confirmed = [h for h in active if self.states[h] == RIGHT]
...
def main():
    ...
    path = "results/test21/results.csv"
